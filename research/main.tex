...
The framework incorporates a lightweight rule-based NLP pipeline optimized for high precision in low-resource conflict text environments designed to extract structured signals from unstructured event descriptions. Specifically, the system performs ontology-based keyword extraction on narrative event notes contained in the dataset \cite{grimmer2013text}.
...
The logical flow for HICE extraction is formalized in Algorithm 1.
...
\subsection{NLP Precision Assessment (HICE Extraction)}
To validate the Health-Impacting Conflict Events (HICE) detection engine, a two-stage manual audit protocol was implemented. First, the automated keyword ontology---integrated with behavioral markers for SDG 16 violations (e.g., looting, arrests)---was applied to the dataset. Second, a randomized sample of $n=200$ flagged incidents was extracted for manual review. Each event narrative was audited against ground-truth reports to confirm whether it represented a legitimate threat to healthcare infrastructure or systemic community well-being. 

The audit revealed a \textbf{Precision of 89.5\%}. This high degree of accuracy is attributed to the contextual filtering logic; by searching for medical keywords exclusively within conflict records, the engine effectively eliminates incidental noise (e.g., non-kinetic mentions of clinics). This confirms that the rule-based NLP pipeline provides a reliable evidence base for humanitarian triage in low-resource text environments.
