...
\begin{enumerate} 
    ...
    \item \textbf{Statistical Anomaly Detection:} To move beyond historical logging and provide predictive utility. Using rolling statistical baselines, the system identifies regional surges in violence ($Z > 1.5$) that deviate from historical means, serving as a leading indicator for humanitarian health crises.
...
\subsection{Statistical Predictive Engine}

The framework transitions from descriptive analysis to predictive analytics through two core statistical models:

\begin{enumerate}
    \item \textbf{Anomaly Detection:} To identify abnormal surges in conflict events, the system computes a regional Z-score based on a 90-day rolling baseline of event frequency \cite{aggarwal_outlier_2017}. The standardized score is defined as:
    \[
    Z = \frac{X - \mu}{\sigma}
    \]
    where $X$ represents the observed number of events in a region during the current period, $\mu$ denotes the mean event frequency over the 90-day baseline, and $\sigma$ represents the corresponding standard deviation. Observations where $Z > 1.5$ are flagged as \textit{Kinetic Anomalies}, indicating a heightened probability of imminent healthcare disruption.

    \item \textbf{Fatality Projection:} Short-term fatality projections are estimated using a least-squares linear regression model applied to the most recent four-week temporal window \cite{cederman_modeling_2014}.
    \[
    y = \beta_0 + \beta_1 x + \epsilon
    \]
    where $y$ represents the projected number of fatalities, $x$ denotes time, $\beta_0$ is the intercept, $\beta_1$ is the slope coefficient estimated using the least-squares method, and $\epsilon$ represents the error term. The model is used to generate a 7-day forward projection to support early warning assessments for humanitarian responders.
\end{enumerate}
...
\subsection{Fatality Trajectory and Risk Projection}
The observatory extends beyond historical documentation by incorporating a predictive analytical framework. Using a least-squares linear regression model applied to a four-week temporal window, the system produces \textbf{seven-day forward projections} of fatality trends. The projection chart (Fig. \ref{fig:fatality_projection}) classifies national conflict momentum as either "Escalating" or "Stable," enabling analysts to estimate short-term humanitarian needs and emergency health service demand.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{assets/fatality_projection.png}
    \caption{Fatality Trajectory and Risk Projection utilizing regression-based forecasting.}
    \label{fig:fatality_projection}
\end{figure}
...
Furthermore, the predictive models employed (Z-score anomaly detection and linear regression) assume a degree of historical continuity. However, sudden political shocks—such as new alliances between EAOs or major military offensives—can deviate sharply from rolling baselines, potentially reducing the accuracy of short-term projections during periods of extreme instability. Finally, the NLP engine, while robust, relies on the quality of unstructured text notes; vague reporting in source media may lead to under-classification of health-impacting events, necessitating manual verification for critical alerts.
...
The \textit{Myanmar Conflict Observatory} establishes a rigorous analytical framework for decoding the complex evolution of political violence in post-coup Myanmar. By synthesizing high-frequency georeferenced data with Natural Language Processing and statistical modeling, this study moves beyond conventional descriptive logging to offer predictive intelligence regarding the intersection of armed conflict and public health security. The findings reveal a definitive structural shift from urban civil disobedience to a prolonged rural insurgency, characterized by a decentralized resistance network and an increasing reliance on remote warfare tactics.
\section{SDG 3 Analysis and Health Risk}
\label{sec:sdg3}
Crucially, the identification of a strong spatial correlation between kinetic engagements and health-impacting incidents underscores the systematic degradation of medical infrastructure. Furthermore, a narrative audit of the dataset reveals a massive "Displacement Tail," with over 3,131 events explicitly documenting civilians fleeing or being displaced, signifying a secondary humanitarian crisis that often exceeds verified fatality counts. The observatory's ability to identify "Red Zones" and detect kinetic anomalies provides a vital mechanism for humanitarian actors, directly contributing to the objectives of SDG 3.d.
